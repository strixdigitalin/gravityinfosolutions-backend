...
const { getJobs, postJob, updateJob, deleteJobImage, deleteJob, deleteAllJobs, sendJobToMail } = require('../controllers/jobController');
...
router.put('/updateJob/:id',  async (req, res) => {
    try {
        let data = await updateJob({ ...req.body, auth: req.user, id: req.params.id });
        if (!data.status) {
            return res.status(400).json(data);
        }
        res.json(data);
